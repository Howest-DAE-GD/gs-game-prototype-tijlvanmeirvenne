...
void Game::Draw( ) const
{
	...
	if (m_ok == gameState::normal || m_ok == gameState::challenge)
	{
		...
		temptimer.Draw(Point2f{ 10.f, 700.f });
	...
	else if (m_ok == gameState::challengeEnd)
	{
		if (m_Score != m_RandomNumber)
		{
			...
			Texture tempText = Texture(scoretext, "Arial.ttf", 50, Color4f{ 0.f,0.f,0.f,1.f });
			tempText.Draw(Point2f(200, 430));
		}
		else
		{
...
{
	int tempscore{ int(pow(3 ,((m_bottles[m_ClickedBottles.x]->GetDecay() + m_bottles[m_ClickedBottles.y]->GetDecay()) / 200))) - 2 };
	if (tempscore > 7777)
	{
		tempscore = 7777;
	}
